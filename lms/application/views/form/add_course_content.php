<link href="<?php echo base_url('assets/datepicker2/daterangepicker.css') ?>" rel="stylesheet" media="all">
<script src="<?php echo base_url('assets/datepicker2/moment.min.js') ?>"></script>
<script src="<?php echo base_url('assets/datepicker2/daterangepicker.js') ?>"></script>
...
<div class="block">
    ...
    <div class="block-content">
        <?= form_open_multipart(base_url('index.php/course/add_content/' . $course_id . '/' . $section_id), 'id="upload-form" role="form" class="form-horizontal"'); ?>
        <div class="row">
            <div class="col-sm-12">
                <div class="row">
...
<?php
$exam_option = array();
$exam_option[''] = 'Select Exam';
$exam_option_string = '<option value="" selected="selected">Select Exam</option>';
foreach ($exams as $exam) {
    $exam_option[$exam->title_id] = $exam->title_name;
    $exam_option_string .= '<option value="' . $exam->title_id . '" >' . $exam->title_name . '</option>';
}
?>
...
<script type="text/javascript">
    $('#upload-media-file').click(function () {
        var type = '<div class="form-group">'
                ...
                + '<input type="radio" value="exam" name="media_type" required="required" onclick="add_media_field(this.value)"> <span>Exam </span>'
                + '<input type="radio" value="live" name="media_type" required="required" onclick="add_media_field(this.value)"> <span>Live Class </span>'
                + '</div>'
                + '</div>';
        $('#media-choose').hide();
...
//Add media fields
    function add_media_field(val) {
        ...
        switch (val) {
            ...
            case 'video':
                ...
                break;
            case 'exam':
                field += '<select name="exam" id="exam_section" class="form-control" required>';
                field += '<?php echo $exam_option_string ?>';
                field += '</select>';
            case 'live':
                $('#media-field').hide();
                $('#media-field2').show();
    ...
    var dNow = new Date();
    var today = (dNow.getMonth()+1) + '/' + dNow.getDate() + '/' + dNow.getFullYear() + ' ' + dNow.getHours() + ':' + dNow.getMinutes();
