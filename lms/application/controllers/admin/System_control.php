...
class System_control extends MS_Controller
{
    ...
    public function view_settings($message = '')
    {
        ...
        if($_POST){
            ...
            if ($this->form_validation->run() != FALSE) {
                ...
                $info['show_latest_courses_on_homepage'] = $this->input->post('show_latest_courses_on_homepage', TRUE);
                $info['number_of_latest_course'] = $this->input->post('number_of_latest_course', TRUE);
                $info['show_latest_exams_on_homepage'] = $this->input->post('show_latest_exams_on_homepage', TRUE);
                $info['number_of_latest_exam'] = $this->input->post('number_of_latest_exam', TRUE);
                $info['student_can_register'] = $this->input->post('student_can_register', TRUE);
                $info['teacher_can_register'] = $this->input->post('teacher_can_register', TRUE);
                $info['teacher_can_create_categories'] = $this->input->post('teacher_can_create_categories', TRUE);
                $info['teacher_can_create_blogs'] = $this->input->post('teacher_can_create_blogs', TRUE);
        ...
        $data['payu_set']  = $this->admin_model->get_payu_settings();
    ...
    public function update_content($message = '')
    {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            {
                move_uploaded_file($_FILES['slider']['tmp_name'], 'slider.png');
            }
    ...
    public function set_payment_gatewaye($name = null, $action = 'install')
    {
        if(!$name) redirect(base_url('index.php/admin/system_control/view_settings'));

        $data['enabled'] = ($action == 'install') ? 1 : 0;;
        ...
        else if($name == 'PayUMoney')
        {
            $exist = $this->db->get_where('payu_settings', array('id'=>1))->row();
            if (!empty($exist)) {
                $this->db->where('id', 1);
                $this->db->update('payu_settings', $data);
            }else{
                $data['id'] = 1;
                $this->db->insert('payu_settings', $data);
           }
        }

        if ($this->db->affected_rows() == 1) {
            $message = ($action == 'install') ? 'Please congure your payment gateway API to complete the installation.' : '';

            $message = '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>' . $name . ' has been ' . $action .'ed successfully. ' . $message . '</div>';
            ...
            $message = '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
                 . 'ERROR! Failed to ' . $action . ' the payment gateway.</div>';
        }
        $this->session->set_flashdata('message', $message);
    ...
    public function update_payu_info()
    {
        if (!$this->session->userdata('log')) {
            show_404 ();
        }
        echo ($this->system_model->update_payu_info())?'TRUE':'FALSE';
    }
    ...
    public function set_system_config()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->db->insert('users',$info)) {
                ...
                $data['teacher_can_register'] = $this->input->post('teacher_can_register', TRUE);
                $info['teacher_can_create_categories'] = $this->input->post('teacher_can_create_categories', TRUE);
                $info['teacher_can_create_blogs'] = $this->input->post('teacher_can_create_blogs', TRUE);
