...
class Admin_control extends MS_Controller {
    ...
    public function create_exam()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->form_validation->set_rules('mock_title', 'Exam Title', 'required|min_length[3]');
            ...
            $this->form_validation->set_rules('random_ques', 'Total Random Question', 'required');

            if ($this->form_validation->run() == FALSE)
            {
                redirect(base_url('index.php/admin_control/create_exam'));
            }
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() !== FALSE)
            {
                ...
                if (isset($_FILES['media']['name'])) {
                    ...
                    $dest = './question-media/'.$this->input->post('media_type').'/'.$_FILES["media"]['name'];
                    if(move_uploaded_file($_FILES["media"]['tmp_name'], $dest))
                    {
                        $file_name = $this->input->post('media_type').'/'.$_FILES["media"]['name'];
                        $file_type = $this->input->post('media_type');
                    }
    ...
    public function update_profile()
    {
        ...
        $this->form_validation->set_rules('paypal_id', 'PayPal Email', 'valid_email');
        if ($this->form_validation->run() == FALSE) {
            ...
            $info['paypal_id'] = $this->input->post('paypal_id', TRUE);
            ...
            if ($this->db->affected_rows() == 1) {
                ...
                $message = '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>Update failed.</div>';
            }
        }

        if($_FILES['avatar']['name'])
        {
            move_uploaded_file($_FILES['avatar']['tmp_name'], './user-avatar/' . $this->session->userdata('user_id') . '.png');
            $message = '<div class="alert alert-success alert-dismissable">'
                        . '<button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
