...
//                ->join('course_sections', 'course_sections.course_id = course_videos.course_id')
    ...
    public function save_course_sections_by_scorm($sections) {
        
        $info = array();
        $info['section_name'] = $sections['section_name'];
        $info['section_title'] = $sections['section_title'];
        $info['course_id'] = $sections['course_id'] ;
        $info['orderList'] = $sections['order_id'] ;
        ...
        $info['video_title'] = $sections['section_title'];;
        $info['preview_type'] = "free";
        $info['content_type'] = "youtube";
        $info['youtube_link'] = str_replace("://", "http://", $sections['youtube_link']);
        $info['uploaded_by'] = $this->session->userdata['user_id'];
        $this->db->insert('course_videos', $info);
        return TRUE;
    }
    
    public function add_course_title_by_scorm($upload_data = '') {
        $info = array();
        $info['course_title'] = $upload_data['course_title'];
        $info['category_id'] = $upload_data['category'];
    ...
    public function update_user_course_status($info) {
        
        $if_exist = $this->db->get_where('user_course_status', array(
        ...
        if ($if_exist) {
            ...
            $this->db->insert('user_course_status', $info);
            return TRUE;
        }
    }
    ...
    public function get_sections_exam($id)
    {
        $result = $this->db->select('course_sections.*,COUNT(section_exams.id) as exams')
                ->order_by('orderList', 'asc')
                ->where('course_id', $id)
                ->from('course_sections')
                ->join('section_exams', 'section_exams.section_id = course_sections.section_id','left')
                ->group_by('course_sections.section_id')
                ->get()
                ->result();
        return $result;
    }
    ...
    public function add_section_exam() {
        $info = array();
        //$info['course_id'] = $this->input->post('course_id', TRUE);
        $info['section_id'] = $this->input->post('section', TRUE);
        $info['exam_id'] = $this->input->post('exam', TRUE);
        $this->db->insert('section_exams', $info);
    ...
    public function add_course_video($link = '', $size = '')
    {
        ...
        if($this->input->post('media_type') == 'exam' && !empty($this->input->post('exam')) ) {
            $inserted_exam_ids =  $this->add_section_exam();
            $section_exam_ids = $this->db->get_where('course_sections', array('section_id' => $info['section_id']))->row()->exam_ids;
            if ($section_exam_ids) {
                $data['exam_ids'] = $section_exam_ids.','.$inserted_exam_ids;
            }else{
               $data['exam_ids'] = $inserted_exam_ids;
            }
